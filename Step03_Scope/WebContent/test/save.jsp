...
<%
	...
	/*
		요청 파라미터 (폼 전송 혹은 GET방식 전송)을 읽어낼때는
		.getParameter() 메소드를 이용한다.
		.getAttrubute() 메소드를 사용하는 실수를 많이 한다. 
		!!!!주의!!!!!
	*/
	String nick=request.getParameter("nick");
	
	//응답 하고 나서 페이지 이동을 해도 읽어낼 수 있도록  HttpSession 객체에 저장을 한다.
	// "nick"이라는 키값으로 String type 저장하기
	session.setAttribute("nick", nick);
	//60초 동안 어떤 요청도 하지 않으면 자동으로 삭제 되도록 설정 (설정을 하지 않으면, 30분으로 설정)
	session.setMaxInactiveInterval(60);
	/*
		session scope (세선영역)에 담은 데이터가 삭제 되는 경우는
